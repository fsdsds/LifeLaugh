...
import com.tony.lifelaugh.Adapter.JokeVideoAdapter;
...
public class JokeVideoFragment extends BaseFragment {
    ...
    private JokeVideoAdapter mJokeVideoAdapter;
    private List<BS_Joke_Video.ShowapiResBodyBean.PagebeanBean.ContentlistBean> mJokeList = new ArrayList<>();
    ...
    public void initData(Bundle savedInstanceState) {
        Log.i(LFLConfig.TAG,"JokePictureFragment-isFirst:"+isFirst);
    ...
    public void getDataFromNet() {
        ...
        Observable<BS_Joke_Video> observable = service.getVideoJoke(LFLConfig.AppId,LFLConfig.AppSercet,Integer.toString(Page),LFLConfig.BS_TYPE_Video);
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<BS_Joke_Video>() {
            @Override
            ...
            @Override
            public void onNext(BS_Joke_Video bs_joke_video) {
                cacheDataToSp(bs_joke_video);//TODO 缓存数据到SP
                setDataInRecycle(bs_joke_video);
    ...
    public void getDataFromSp() {
        ...
        if(mJokeSp != null){
            Log.i(LFLConfig.TAG,"getDataFromSp-缓存："+mJokeSp.getString("JokeVideoJson"+Page));
            Log.i(LFLConfig.TAG,"getDataFromSp-页数："+mJokeSp.getInt("Page"));
            //TODO 得到缓存的页数
            Page = mJokeSp.getInt("Page");
            //TODO 取得所有页面缓存
            for (int i = 1; i <= Page; i++) {
                BS_Joke_Video bean = (BS_Joke_Video) JsonUtils.toObject(mJokeSp.getString("JokeVideoJson"+i),BS_Joke_Video.class);
                mJokeList.addAll(bean.getShowapi_res_body().getPagebean().getContentlist());
            }

            mJokeVideoAdapter = null;
            mHeaderAndFooterRecyclerViewAdapter = null;
            if(mJokeVideoAdapter == null){
                mJokeVideoAdapter = new JokeVideoAdapter(mActivity,mJokeList);
            }

            if(mHeaderAndFooterRecyclerViewAdapter == null){
                mHeaderAndFooterRecyclerViewAdapter = new HeaderAndFooterRecyclerViewAdapter(mActivity,mJokeVideoAdapter);
     ...
     * @param bs_joke
     */
    private void cacheDataToSp(BS_Joke_Video bs_joke) {

        if(mJokeSp == null){
            mJokeSp = new SPUtils(mActivity,"BS_Joke_Video");//TODO 初始化SP缓存
        }else{
            //TODO 如果sp存在，在刷新的时候要先清除掉Sp中的缓存
            if(isRefresh == 1){
                int a = mJokeSp.getInt("Page");
                for (int i = 0; i < a; i++) {
                    mJokeSp.putString("JokeVideoJson"+i,"");
                    mJokeSp.putInt("Page",1);
                }
            }
        }
        //TODO 缓存每一页加载的json数据
        mJokeSp.putString("JokeVideoJson"+Page, JsonUtils.toJson(bs_joke,1));//TODO 把json存进SP文件
        //TODO 缓存最大页码
        mJokeSp.putInt("Page",Page);
        Log.i(LFLConfig.TAG,"setDataToSp-缓存："+mJokeSp.getString("JokeVideoJson"+Page));
     ...
     * @param bs_joke
     */
    private void setDataInRecycle(BS_Joke_Video bs_joke) {
        ...
        if(mJokeVideoAdapter == null){
            ...
            mJokeVideoAdapter.notifyDataSetChanged();
        }
        if(mHeaderAndFooterRecyclerViewAdapter == null){
            mHeaderAndFooterRecyclerViewAdapter = new HeaderAndFooterRecyclerViewAdapter(mActivity,mJokeVideoAdapter);
            mLRecyclerView.addItemDecoration(new DividerItemDecoration(mActivity));
