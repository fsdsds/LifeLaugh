...
import com.tony.lifelaugh.Adapter.JokePictureAdapter;
...
public class JokePictureFragment extends BaseFragment {
    ...
    private List<BS_Joke_Picture.ShowapiResBodyBean.PagebeanBean.ContentlistBean> mJokeList = new ArrayList<>();
    public static JokePictureFragment mJokePictureFragment;
    public JokePictureAdapter mJokePictureAdapter;
    ...
    public void getDataFromNet() {
        ...
        Observable<BS_Joke_Picture> observable = service.getPictureJoke(LFLConfig.AppId,LFLConfig.AppSercet,Integer.toString(Page),LFLConfig.BS_TYPE_Picture);
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<BS_Joke_Picture>() {
            @Override
            ...
            @Override
            public void onNext(BS_Joke_Picture bs_joke_picture) {
                cacheDataToSp(bs_joke_picture);//TODO 缓存数据到SP
                setDataInRecycle(bs_joke_picture);
     ...
     * @param bs_joke
     */
    private void setDataInRecycle(BS_Joke_Picture bs_joke) {
        ...
        if(isFirst){
            mJokePictureAdapter =null;
            mHeaderAndFooterRecyclerViewAdapter = null;
        }
        if(mJokePictureAdapter == null){
            mJokePictureAdapter = new JokePictureAdapter(mActivity,mJokeList);
        }else{
            mJokePictureAdapter.notifyDataSetChanged();
        }
        if(mHeaderAndFooterRecyclerViewAdapter == null){
            mHeaderAndFooterRecyclerViewAdapter = new HeaderAndFooterRecyclerViewAdapter(mActivity,mJokePictureAdapter);
    ...
    public void getDataFromSp() {
        ...
        if(mJokeSp != null){
            Log.i(LFLConfig.TAG,"getDataFromSp-缓存："+mJokeSp.getString("JokePictureJson"+Page));
            Log.i(LFLConfig.TAG,"getDataFromSp-页数："+mJokeSp.getInt("Page"));
            //TODO 得到缓存的页数
            Page = mJokeSp.getInt("Page");
            //TODO 取得所有页面缓存
            for (int i = 1; i <= Page; i++) {
                BS_Joke_Picture bean = (BS_Joke_Picture)JsonUtils.toObject(mJokeSp.getString("JokePictureJson"+i),BS_Joke_Picture.class);
     ...
     * @param bs_joke
     */
    private void cacheDataToSp(BS_Joke_Picture bs_joke) {

        if(mJokeSp == null){
            mJokeSp = new SPUtils(mActivity,"BS_Joke_Picture");//TODO 初始化SP缓存
        }else{
            //TODO 如果sp存在，在刷新的时候要先清除掉Sp中的缓存
            if(isRefresh == 1){
                int a = mJokeSp.getInt("Page");
                for (int i = 0; i < a; i++) {
                    mJokeSp.putString("JokePictureJson"+i,"");
                    mJokeSp.putInt("Page",1);
                }
            }
        }
        //TODO 缓存每一页加载的json数据
        mJokeSp.putString("JokePictureJson"+Page, JsonUtils.toJson(bs_joke,1));//TODO 把json存进SP文件
        //TODO 缓存最大页码
